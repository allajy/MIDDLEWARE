...
import com.bxc.assemble.dynamicTask.engine.TaskJobCompleter;
...
public class JobCompleteHelper {
			...
			public void run() {
				...
				while (!toStop) {
					try {
						...
						List<Long> losedJobIds  = ScheduleConfiguration.getAdminConfig().getXxlJobLogDao().findLostJobIds(losedTime);
						...
						if (losedJobIds!=null && losedJobIds.size()>0) {
							for (Long logId: losedJobIds) {
								...
								TaskJobCompleter.updateHandleInfoAndFinish(jobLog);
							}
	...
	private ReturnT<String> callback(HandleCallbackParam handleCallbackParam) {
		// valid log item
		JobLog log = ScheduleConfiguration.getAdminConfig().getXxlJobLogDao().load(handleCallbackParam.getLogId());
		if (log == null) {
		...
		TaskJobCompleter.updateHandleInfoAndFinish(log);
